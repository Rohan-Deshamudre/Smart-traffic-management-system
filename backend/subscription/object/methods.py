...
def create_subscriber_object(reply_channel, message_obj):
    ...
    road_segments = (
        {}
        if road_segments_key not in message_obj
        else create_road_segments(message_obj[road_segments_key])
    )
...
def create_simulation_object(sim_id, road_segments):
    ...
    if sim_id == "live":
        ...
        name = "Recorded live simulation %s" % str(datetime.now(pytz.utc).isoformat())
...
def create_road_segments(road_segments):
    ...
    for road_segment_id in road_segments:
        try:
            rs_map[road_segment_id] = get_polylines_with_id(road_segment_id)
