...
from api.road_conditions.methods.getter import get_road_condition_date_with_id, \
...
keys = [name_key, time_key, value_key, type_key,
        road_conditions_key, actions_key]
...
def to_json_road_condition_time(road_condition_id: int):
    if has_road_condition_date_with_id(road_condition_id):
        road_condition_time = get_road_condition_date_with_id(
            road_condition_id)
        time_object = {
...
def to_json_road_condition(road_condition: RoadCondition):
    road_condition_object = {name_key: road_condition.name,
                             time_key: to_json_road_condition_time(
                                 road_condition.id),
...
def import_road_conditions(json_object, road_segment_id, road_condition_id):
    if all((w in json_object for w in keys)):
        ...
        road_condition = create_road_condition(name, time, value,
