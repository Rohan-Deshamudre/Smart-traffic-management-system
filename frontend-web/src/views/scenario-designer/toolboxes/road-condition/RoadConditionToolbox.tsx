...
import RoadConditionButton from "../../../../components/buttons/RoadConditionButton";
...
class RoadConditionToolbox extends React.Component<Props, State> {
    ...
    handleRoadConditionLevel(newLevel: string) {
        this.setState({
            saved: false,
            level: newLevel
        }, () => this.disabled());
...

                {
                    this.state.roadConditionTypeId == 7 ? (
                        <CongestionLevelButton level={this.state.level} handleCongestionLevel={this.handleCongestionLevel}
                            ...
                            [10, 11, 12].includes(this.state.roadConditionTypeId) ? (
                                <RoadConditionButton road_type={this.state.roadConditionTypeId} level={this.state.level}
                                    handleRoadConditionLevel={this.handleRoadConditionLevel} disabled={this.props.readOnly} />
                            ) : null
                        )
                }
                {!this.props.readOnly && (
